This data consists of two multi-turn adversarial conversations generated by human raters interacting with a dialog model and then rated with safety labels. It contains 350 dialogs rated by a diverse rater pool of 123 unique raters. Each conversation is rated with five safety top-level categories and one overall comprehension question of the conversation. Raters were recruited were balanced by gender (man or woman), race/ethnicity (White, Black, Latine, Asian, Multiracial) and each rater rated all items. The total rows in this dataset are: 43050.

Field 1 is an id unique to each rater, conversation pair
Fields 2-5 are rater metadata
Fields 6-10 are conversation metadata
Fields 11-14 are expert annotations related to safety and degree of harm, including gold labels
Fields 15-34 are granular safety ratings from individual raters
